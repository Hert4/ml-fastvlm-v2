# LLaVA Ouro Model for HuggingFace
# Ouro: Looped Language Model by ByteDance
...
def build_vision_projector(config, delay_load=False, **kwargs):
    ...
    mm_hidden_size = getattr(config, "mm_hidden_size", 3072)  # MobileCLIP output
    hidden_size = config.hidden_size  # Ouro-1.4B: 2048
...
# ============== Ouro Base Model Loader ==============

def load_ouro_base_model(model_name="ByteDance/Ouro-1.4B"):
    """Load Ouro base model from HuggingFace."""
    # Ouro requires trust_remote_code=True
...
class LlavaMetaModel:
    ...
    def init_vision_modules(self, config):
...
class LlavaOuroModel(PreTrainedModel, LlavaMetaModel):
    """LLaVA Ouro Model - combines Ouro LLM with vision capabilities."""
    ...
    def __init__(self, config: LlavaOuroConfig):
        ...
        # Load Ouro base model components
        ...
        # Ouro transformer layers (will be loaded from pretrained)
        self.layers = None  # Placeholder - loaded from pretrained
        self.norm = None    # Placeholder - loaded from pretrained
    ...
    def forward(self, *args, **kwargs):
        # This will be overridden when loading from pretrained
        raise NotImplementedError("Use LlavaOuroForCausalLM.from_pretrained()")


class LlavaOuroForCausalLM(PreTrainedModel, LlavaMetaForCausalLM):
    """LLaVA Ouro for Causal Language Modeling with vision capabilities."""
    ...
    def __init__(self, config: LlavaOuroConfig):
        ...
        # Initialize base Ouro model
        self._init_ouro_model(config)
    ...
    def _init_ouro_model(self, config):
        """Initialize Ouro model structure."""
        # Create a simple model structure that matches Ouro
        self.model = type('OuroModel', (), {
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        """Load pretrained LLaVA Ouro model."""
        config = kwargs.pop("config", None)

        if config is None:
            config = LlavaOuroConfig.from_pretrained(pretrained_model_name_or_path, **kwargs)

        # Check if this is a LLaVA model or base Ouro
        is_llava = getattr(config, "mm_vision_tower", None) is not None

        if is_llava:
            # Load as LLaVA model
            model = super().from_pretrained(pretrained_model_name_or_path, config=config, *args, **kwargs)
        else:
            # Load base Ouro and convert to LLaVA
            print("Loading base Ouro model and converting to LLaVA...")
            ...
            # Create LLaVA config from Ouro config
            llava_config = LlavaOuroConfig(
                ...
                num_key_value_heads=getattr(base_model.config, "num_key_value_heads", base_model.config.num_attention_heads),
                mm_vision_tower="mobileclip_l_384",
                mm_hidden_size=3072,
                mm_projector_type="mlp2x_gelu",
                total_ut_steps=getattr(base_model.config, "total_ut_steps", 4),
                early_exit_threshold=getattr(base_model.config, "early_exit_threshold", 1.0),
            )

            # Create LLaVA model
            model = cls(llava_config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Call the underlying Ouro model
        if hasattr(self.model, 'forward'):
            ...
            # Handle Ouro's multi-output format
