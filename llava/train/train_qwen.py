...
def preprocess_qwen_3(
    ...
    """
    Preprocess function for Qwen3 models.
    Qwen3 uses the same ChatML format as Qwen2.
    Note: When using tokenizer.apply_chat_template with Qwen3,
    use enable_thinking=False to disable thinking mode.
    """
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
    ...
    # Qwen3 uses QWEN_3 separator style which is same format as QWEN_2
    ...
    for conversation, target in zip(conversations, targets):
        ...
        rounds_before = conversation.split(conv.sep)
        ...
        for i, rou in enumerate(rounds):
            ...
            round_len += 2  # this is to compensate for the sep2
...
def preprocess(
    ...
    # Qwen3 support - uses same preprocessing as Qwen2 (ChatML format)
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            if "qwen3" in model_name_lower or "qwen/qwen3" in model_name_lower:
                ...
                if LlavaQwen3ForCausalLM is None:
        ...
        # Check if using Qwen3 model for non-vision models
        model_name_lower = model_args.model_name_or_path.lower()
        if "qwen3" in model_name_lower or "qwen/qwen3" in model_name_lower:
